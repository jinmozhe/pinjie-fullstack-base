...
describe("admin access mapping", () => {
  it("denies every protected area without a current administrator", () => {
    expect(access({ settings })).toEqual({ canUsers: false, canAdmins: false, canRoles: false, canSecurity: false });
  });
  ...
  it("maps ordinary read permissions and grants every area to superusers", () => {
    expect(access(state(["users:read", "admins:read", "roles:read"]))).toEqual({
      canUsers: true,
      canAdmins: true,
      canRoles: true,
      canSecurity: false,
    });
    expect(access(state([], true))).toEqual({ canUsers: true, canAdmins: true, canRoles: true, canSecurity: true });
  });
});
