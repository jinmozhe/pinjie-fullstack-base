import type { ActionResult, SessionRead, UserAuthSessionOut, UserPrincipalOut, UserRegisterIn, UserUpdateIn } from "@pinjie/api-client";
...
export const webAuthApi = {
  ...
  me: () => webRequest<UserPrincipalOut>("/api/v1/users/me"),
  update: (input: UserUpdateIn) => webRequest<UserPrincipalOut>("/api/v1/users/me", { method: "PATCH", body: JSON.stringify(input) }),
  changePassword: (currentPassword: string, newPassword: string) => webRequest<ActionResult>("/api/v1/users/me/password", { method: "POST", body: JSON.stringify({ current_password: currentPassword, new_password: newPassword }) }),
  sessions: () => webRequest<SessionRead[]>("/api/v1/users/me/sessions"),
