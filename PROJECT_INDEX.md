...
| 派生类型 | 无 |
| 母版基线 | 当前仓库 |
| 当前阶段 | v0.1.1 不可变发布与三端 GHCR 镜像发布已完成，GHCR/TCR 双仓真实发布已验证，生产部署未触发；第四次 CNB Run 已通过上下文校验和 TCR 登录，临时 BuildKit 容器因托管 Docker 权限限制无法启动，正在改用默认 Buildx 驱动和 Run 唯一候选标签；旧版管理员确认端点限时保留至 2026-09-26 |
...
| 计划 | 状态 | 说明 |
| --- | --- | --- |
| [CNB 云原生构建与 TCR 单仓镜像发布计划](plans/2026-08-31_TCR个人版双仓镜像发布与腾讯云内网部署计划.md) | 实施中 | 第四次 CNB Run 已确认托管 Docker 禁止嵌套 BuildKit，正在改用默认 Buildx 驱动和 Run 唯一候选标签；三镜像尚未发布，生产部署继续锁定 |
