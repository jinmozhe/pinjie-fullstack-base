...
| 派生类型 | 无 |
| 母版基线 | 当前仓库 |
| 当前阶段 | v0.1.1 不可变发布与三端 GHCR 镜像发布已完成，GHCR/TCR 双仓真实发布已验证，生产部署未触发；GitHub 固定 SHA 交接、CNB 三镜像构建扫描和 TCR 单仓发布已完成本地检查点，CNB 密钥仓库已配置且未绑定付费预算，等待推送、远端验证与生产边界确认；旧版管理员确认端点限时保留至 2026-09-26 |
...
| 计划 | 状态 | 说明 |
| --- | --- | --- |
| [CNB 云原生构建与 TCR 单仓镜像发布计划](plans/2026-08-31_TCR个人版双仓镜像发布与腾讯云内网部署计划.md) | 实施中 | 已验证双仓流程保留为回滚基线，本地源码交接与 CNB 发布链路已形成检查点，密钥仓库已配置且未绑定付费预算，等待推送和远端验证；生产部署继续锁定 |
