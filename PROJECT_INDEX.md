...
| 派生类型 | 无 |
| 母版基线 | 当前仓库 |
| 当前阶段 | v0.1.1 不可变发布与三端 GHCR 镜像发布已完成，GHCR/TCR 双仓真实发布已验证，生产部署未触发；首次 CNB 三镜像构建、扫描、证据和 TCR 单仓不可变发布已成功，正在以第二个 Commit 验证持久缓存；旧版管理员确认端点限时保留至 2026-09-26 |
...
| 计划 | 状态 | 说明 |
| --- | --- | --- |
| [CNB 云原生构建与 TCR 单仓镜像发布计划](plans/2026-08-31_TCR个人版双仓镜像发布与腾讯云内网部署计划.md) | 实施中 | 首次 CNB 三镜像构建、Trivy、SBOM、provenance、TCR SHA 标签和结构化证据已成功，等待第二个 Commit 验证缓存；生产部署继续锁定 |
