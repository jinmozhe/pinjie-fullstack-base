...
| 派生类型 | 无 |
| 母版基线 | 当前仓库 |
| 当前阶段 | v0.1.1 不可变发布、GitHub 固定 SHA 验证与交接、CNB 三镜像构建扫描、TCR 单仓不可变发布和跨 Commit Registry 缓存复用均已验证；Admin 运行镜像全量升级与 CNB 失败证据链已通过真实发布复验；本地生产编排已改为接入 1Panel 共享 PostgreSQL 与 Redis，生产迁移和部署未触发；旧版管理员确认端点限时保留至 2026-09-26 |
