...
| 派生类型 | 无 |
| 母版基线 | 当前仓库 |
| 当前阶段 | v0.1.1 不可变发布与三端 GHCR 镜像发布已完成，GHCR/TCR 双仓真实发布已验证，生产部署未触发；GitHub 固定 SHA 交接与新 SHA Full Validation 已通过，第二次 CNB Run 的 Prepare 已通过但发布上下文校验失败，正在修复无效工具门禁和静默诊断；旧版管理员确认端点限时保留至 2026-09-26 |
...
| 计划 | 状态 | 说明 |
| --- | --- | --- |
| [CNB 云原生构建与 TCR 单仓镜像发布计划](plans/2026-08-31_TCR个人版双仓镜像发布与腾讯云内网部署计划.md) | 实施中 | 新 SHA Full Validation 与源码交接已通过；第二次 CNB Run 的 Prepare 已通过，发布上下文校验因无效 Node 前置门禁或静默断言失败，正在修复并准备再次交付；生产部署继续锁定 |
