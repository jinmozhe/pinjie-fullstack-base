...
| 派生类型 | 无 |
| 母版基线 | 当前仓库 |
| 当前阶段 | v0.1.1 不可变发布与三端 GHCR 镜像发布已完成，GHCR/TCR 双仓真实发布已验证，生产部署未触发；GitHub 固定 SHA 交接已通过真实验证，首次 CNB Run 因锁时长类型错误在 Prepare 阶段失败，已完成本地修复并等待重新交付验证；旧版管理员确认端点限时保留至 2026-09-26 |
...
| 计划 | 状态 | 说明 |
| --- | --- | --- |
| [CNB 云原生构建与 TCR 单仓镜像发布计划](plans/2026-08-31_TCR个人版双仓镜像发布与腾讯云内网部署计划.md) | 实施中 | GitHub 源码交接已真实通过；首次 CNB Run 在 Prepare 阶段失败，锁时长字段已按官方语法完成本地修复，等待重新交付验证；生产部署继续锁定 |
