...
| 派生类型 | 无 |
| 母版基线 | 当前仓库 |
| 当前阶段 | v0.1.1 不可变发布与三端 GHCR 镜像发布已完成，GHCR/TCR 双仓真实发布已验证，生产部署未触发；三个不同 Commit 的 CNB 三镜像发布已成功，固定构建时间戳的缓存基线已建立，正在复验下一 Commit 的跨 Run 缓存命中；旧版管理员确认端点限时保留至 2026-09-26 |
...
| 计划 | 状态 | 说明 |
| --- | --- | --- |
| [CNB 云原生构建与 TCR 单仓镜像发布计划](plans/2026-08-31_TCR个人版双仓镜像发布与腾讯云内网部署计划.md) | 实施中 | 两个不同 Commit 的构建、扫描、证据和 TCR 发布已成功，正在验证固定构建时间戳后的 Registry 缓存；生产部署继续锁定 |
