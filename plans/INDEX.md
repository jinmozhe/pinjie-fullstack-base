...
| 路径 | 状态 | 结果 | 影响范围 | 用途 |
| --- | --- | --- | --- | --- |
| `plans/2026-08-31_TCR个人版双仓镜像发布与腾讯云内网部署计划.md` | 实施中 | 不适用 | Deployment、Documentation | GitHub 固定 SHA 交接、CNB 三镜像构建扫描、TCR 单仓不可变发布和结构化证据已完成首次真实验证，等待第二个 Commit 缓存验证与生产边界确认 |
