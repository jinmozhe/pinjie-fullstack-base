...
| 路径 | 状态 | 结果 | 影响范围 | 用途 |
| --- | --- | --- | --- | --- |
| `plans/2026-08-30_依赖复现与发布验证闭环整改计划.md` | 实施中 | 不适用 | Backend、Admin、Web、Deployment、Documentation | 固定 uv 依赖复现工具链，建立同 Commit SHA 的完整验证证据与镜像发布门禁，并修复首次远端验收暴露的 Origin 配置错配 |
