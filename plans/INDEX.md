...
| 路径 | 状态 | 结果 | 影响范围 | 用途 |
| --- | --- | --- | --- | --- |
| `plans/2026-09-16_FullValidation分层验证计划.md` | 已结束 | 已完成；模式、证据与文档已同步，轻量门禁通过，已推送功能分支；真实重型验证未执行 | CI、Admin/Web 验证编排、证据与文档 | 增加手动 full/smoke 范围，隔离证据并保留 strict/fast 交接 |
| `plans/2026-09-14_pnpm补丁缓存验证治理计划.md` | 已结束 | 已完成；根规则、pnpm 运维指南和真实消费者解析检查已同步，轻量门禁通过 | Admin、依赖治理与文档 | 将 pnpm 补丁验证收紧为锁文件、冻结安装和真实消费者解析 |
| `plans/2026-09-14_中危前端依赖修复计划.md` | 已结束 | 已完成；安全版本、CommonJS 解码器补丁和轻量门禁通过，远端告警待 Dependabot 重扫确认 | Admin、Web、依赖治理与文档 | 修复 10 条中危依赖告警及安全版本迁移 |
| `plans/2026-09-14_母版发布链路可靠性回馈计划.md` | 已结束 | 已完成；母版代码与文档回馈、治理及三端轻量验证通过，真实云端发布未执行 | Backend 镜像、发布诊断、运维文档 | 回馈 Nav 的平台连接、缓存与故障定位经验 |
| `plans/2026-09-11_Markdown工具依赖安全修复计划.md` | 已结束 | 已完成；smol-toml 安全版本、根锁文件和治理门禁已同步，PR #53 的 13 项必需检查通过 | 根开发依赖、治理与文档 | 修复 smol-toml 高危漏洞导致的线上 Node 审计阻断 |
