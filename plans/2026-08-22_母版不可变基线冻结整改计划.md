...
- 状态：已结束
- 结果：P1-01 至 P1-09 与同步处理项已完成，三端质量、真实 PostgreSQL/Redis、公开契约、容器、桌面与移动跨栈 E2E、文档和治理门禁均通过。当前工作区满足不可变母版冻结候选条件；尚未执行需要独立授权的提交或不可变 Tag，因此不能表述为已创建不可变基线。
...
- `plans/2026-08-22_母版不可变基线冻结整改计划.md`
...
- [x] Backend Profile Origin、Cookie 和 API 路径跨端访问被服务端拒绝，合法 Web/Admin 流程通过。
- [x] 最后超级管理员真实 PostgreSQL 并发测试证明最多一个竞争操作成功。
- [x] Redis 清理失败不会翻转已提交认证成功，且失败具有日志和指标上下文。
- [x] Session/Refresh Token 分页、上限、dry-run 和 `--apply` 清理测试通过。
- [x] 不同数据库 Session 不共享事务上下文，同 Session 非法嵌套明确失败。
- [x] OpenAPI 导出与根契约一致，API Client 重新生成无漂移，Admin/Web 类型检查通过。
- [x] Access 过期只刷新一次；Logout 失败可重试；改密保留当前登录并轮换 Cookie。
- [x] Admin 各日志 Tab 独立授权，无权限辅助 Query 不执行，列表使用服务端分页。
- [x] Web BFF allowlist、Metadata/canonical 和两端安全响应头测试通过。
- [x] Umi 使用上游声明的 Vite 版本，冻结安装、Admin 测试和 production build 通过。
- [x] 所有生产基础镜像和基础设施镜像固定完整 digest，Compose 正反例门禁通过。
- [x] Admin/Web 高风险 HTTP、认证和 BFF 代码纳入覆盖率门禁。
- [x] Backend 全量静态检查、真实 PostgreSQL/Redis pytest 和覆盖率通过。
- [x] Admin/Web lint、typecheck、Vitest、覆盖率和 production build 分项通过。
- [x] 桌面与移动端真实跨栈 Browser E2E 通过，或如实记录外部环境阻断而不宣称冻结完成。
- [x] `pnpm lint:md`、`pnpm check:workspace`、`pnpm check:boundaries`、`pnpm check:governance` 和 `git diff --check` 通过。
- [x] 最终工作区只包含本计划范围内的预期修改，无缓存、构建产物、测试服务或临时文件残留。
...
- Backend 完成 Web/Admin Cookie Profile、Origin 与 API 路径隔离；Session 接口改为受控分页，增加保留策略及默认 dry-run、显式 `--apply` 的清理工具。
- 最后超级管理员保护使用 PostgreSQL 事务级 advisory lock 串行化；事务上下文绑定真实 `AsyncSession`；认证提交后的 Redis 限流清理改为可观测 best-effort，派生状态失败不再翻转权威提交结果。
- Admin 与 Web 统一为单次 Refresh 和一次原请求重放，Logout 失败保留当前页面并展示错误，改密保留当前会话并轮换 Cookie。
- Admin 安全日志按类别独立授权并使用服务端分页；Web BFF 建立路径、方法和 Cookie 白名单；Admin Nginx 同步限制管理路径，两端均补齐安全响应头。
- Web Metadata 改为运行时生成，canonical 使用部署时 `WEB_PUBLIC_ORIGIN`；登录和注册标题不再重复品牌名。
- 移除 Umi 内部 Vite 跨主版本 override，Umi 使用 `vite@4.5.2`，Admin Vitest 独立使用 `vite@6.4.3`；唯一根锁文件重新解析并通过供应链策略。
- Backend、Admin、Web、PostgreSQL 与 Redis 基础镜像全部固定完整 digest；生产 Compose 配置、正例和可变 Tag、短 digest、缺失变量等反例门禁通过。
- Backend 共 119 项 pytest 通过，覆盖率 90.86%；Ruff、format、Mypy、import-linter、Alembic 与 OpenAPI 导出通过。根契约包含 39 条路径、47 个操作，API Client 已重新生成且无漂移。
- Admin 共 33 项 Vitest 通过，语句覆盖率 90.60%、分支 86.24%、函数 86.73%、行 94.40%；Web 共 26 项 Vitest 通过，语句和行覆盖率 90.46%；两端 typecheck、lint 和 production build 均通过。
- 标准冷启动 `pnpm test:e2e` 共 8 项通过，另 8 项因桌面与移动项目隔离按预期跳过；Web standalone 在 Windows 上已归一错误目录符号链接并通过冷启动。
- Browser 实测覆盖 Web/Admin 桌面与 390×844 移动视口，无横向溢出、遮挡或控制台错误。三张本地生产镜像构建成功且均为非 Root 用户；Backend 应用可导入，Web `/login` 与 Admin `/healthz` 运行时检查通过，canonical 和两端安全响应头符合契约。
- `pnpm check:workspace`、`pnpm check:boundaries`、`pnpm check:governance`、`pnpm lint:md` 与最终差异卫生检查通过；项目结构统计同步为 321 个项目文件、68 个含文件目录。

## 剩余问题

- 本计划没有未完成的技术整改。Umi 固定依赖链中的 React Router `6.3.0` 两条 Medium 告警和 `elliptic 6.6.1` Low 告警没有兼容修复版本，已由既有依赖治理计划建立截至 2026-09-21 的限时风险接受，仍须按期复核，不能表述为漏洞已经修复。
- 当前工作区尚未提交，也未创建不可变 Git Tag。只有在用户分别授权提交和 Tag 后，才能从“冻结候选”转为可跨系统追溯的不可变母版基线；发布镜像和部署继续属于独立授权动作。
