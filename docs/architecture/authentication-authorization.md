...
角色权限配置继续消费 `GET /api/v1/admin/permissions` 返回的扁平源码权限目录，并向角色权限写接口提交 `permission_codes: string[]`。Admin 在界面层按权限码资源前缀构建 Ant Design TreeSelect 分组，支持父子联动、名称和代码搜索；未知前缀进入“其他权限”，停用权限保持可见但不可新增选择。分组节点只使用前端内部值，提交前按当前权限目录过滤，因此后端契约、权限码权威来源和服务端授权边界不依赖前端树结构。
