...
| 文档收尾、交接、PR 前审计 | 读取文档索引、活动计划、Changelog 和影响矩阵，执行知识同步 |
...
| 主要内容 | 项目身份、当前阶段、活动计划、系统状态、权威来源和派生入口 |
...
### 10.5 `docs/PROJECT_REQUIREMENTS.md`
...
| 索引中匹配的活动计划 | 任何继续实施、评审或验证该目标的任务 | 当前确认范围、实施顺序、风险、验证和待确认问题 |
...
### 10.11 Changelog、README、契约和实际文件
...
| 提交或推送 | 根规则、当前计划、Git 差异 | Changelog、PR 模板 | 使用现有任务计划 | 提交和推送分别需要用户明确授权 |
...
| Git 暂存、提交、推送 | 各动作按用户明确要求执行 | Git 差异、计划和交付状态 |
...
6. 本任务是否需要计划和用户确认？
7. 哪些动作需要独立授权？
8. 预计影响 Backend、Admin、Web、API Client、Database、Deployment 中哪些范围？
9. 计划运行哪些验证，哪些当前没有执行条件？
...
- [ ] 已明确报告失败、跳过、未适用和未执行项。
- [ ] 已同步契约、迁移、测试和相关权威文档。
- [ ] 已回写原计划、索引和 Changelog。
- [ ] 已清理本次验证产生且可安全识别的缓存、服务、端口和临时产物。
- [ ] 未读取、提交或输出真实秘密与生产数据。
- [ ] 未自动执行提交、推送、Tag、镜像发布、部署或生产变更。
