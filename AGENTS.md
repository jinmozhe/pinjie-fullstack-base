...
- 单次任务的范围、确认、实施和验证进入原 `plans/*.md`；长期强制约束进入对应 `AGENTS.md`；当前状态进入 `PROJECT_INDEX.md`；已交付事实进入 `CHANGELOG.md`。
...
- `plans/` 中每份现存 Markdown 文档必须在 `PROJECT_INDEX.md` 中保留索引记录，计划结束后不得删除索引行。
...
- 普通提交的 Commit SHA 不重复写入 `CHANGELOG.md`、`plans/*.md` 或 `PROJECT_INDEX.md`。这些文档分别记录已交付变化、实施背景与验证、当前事实与导航。
