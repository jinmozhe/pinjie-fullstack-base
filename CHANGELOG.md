...
- 将 Admin 角色权限配置升级为 Ant Design TreeSelect：扁平源码权限目录在前端按用户、管理员、角色与权限、安全与系统、文件资产分组，支持父子联动、名称与权限码搜索、已有权限回填和响应式标签；停用权限保持可见但不可新增选择，未知权限进入“其他权限”，保存前只保留后端目录中的真实权限码。
...
- 增加 Admin 创建普通用户与公开注册开关联动：新增受 `users:create`、管理员会话、CSRF 和审计保护的创建端点，管理员手动填写初始密码并可决定是否允许登录，创建过程不产生 Web 会话；Web 通过公共能力契约服从 Backend 的 `REGISTRATION_MODE`，关闭时隐藏注册入口并重定向，能力不可用时失败关闭。
...
- Admin 登录表单输入框改为低对比度细边框并移除内阴影，输入内容、占位文字和登录按钮文字统一使用 `14px / 400`；登录按钮改为浅蓝底、深蓝字并保留明确的 hover、active 与焦点反馈。
...
- Admin 登录页按参考页面调整为无卡片居中窄表单、项目品牌标识、低透明度蓝青红三色宽幅斜向漫射背景和单行页脚；背景使用固定 CSS 渐变且不随机变化，保留真实用户名密码认证、错误反馈和安全 Cookie 会话，移除无效的自动登录复选框、单标签 Tabs 与第三方登录图标，并将“忘记密码？”改为联系超级管理员重置密码的悬浮提示。
